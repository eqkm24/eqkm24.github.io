  ...
  mining: {
    skills: [
      { name:'단련된 곡괭이', type:'P', tier:'Lv.1',
        ...
        cols:['레벨','데미지 증가'],
        rows: Array.from({length:20},(_,i)=>[i+1, `${(i+1)*5}%`]) },
      { name:'광맥 감각', type:'P', tier:'Lv.1',
        desc:'채광 시 광물이 추가 드롭될 확률이 생깁니다.',
        cols:['레벨','추가 드롭 확률'],
        rows: Array.from({length:20},(_,i)=>[i+1, `${i+1}%`]) },
  ...
  fishing: {
    skills: [
      ...
      { name:'소문난 미끼', type:'P', tier:'Lv.1',
        ...
        cols:['레벨','추가 낚시 확률'],
        rows: Array.from({length:20},(_,i)=>[i+1, `${(i+1)*2}%`]) },
      { name:'낚싯줄 장력', type:'P', tier:'Lv.1',
        desc:'일반 등급 비율이 감소하여 고급·희귀 물고기를 더 자주 낚습니다.',
        cols:['레벨','일반 감소','고급↑ 증가'],
        rows: Array.from({length:20},(_,i)=>[i+1, `-${i+1}%`, `+${i+1}%`]) },
  ...
  farming: {
    skills: [
      { name:'개간의 서약', type:'P', tier:'Lv.1',
        ...
        cols:['레벨','화분통 추가'],
        rows: Array.from({length:20},(_,i)=>[i+1, `+${(i+1)*3}개`]) },
      { name:'풍년의 축복', type:'P', tier:'Lv.1',
        desc:'1등급 확률을 높이고 황금 작물 드롭률도 증가합니다.',
        cols:['레벨','1등급 증가','황금 드롭'],
        rows: Array.from({length:20},(_,i)=>[i+1, `+${i+1}%`, `+${((i+1)*0.5).toFixed(1)}%`]) },
  ...
  cooking: {
    skills: [
      { name:'손질 달인', type:'P', tier:'Lv.1',
        ...
        cols:['레벨','제작 시간 감소'],
        rows: Array.from({length:20},(_,i)=>[i+1, `${(i+1)*3}%`]) },
      { name:'맛의 균형', type:'P', tier:'Lv.1',
        desc:'음식 효과 유지시간이 레벨에 비례하여 증가합니다.',
        cols:['레벨','효과 시간 증가'],
        rows: Array.from({length:20},(_,i)=>[i+1, `+${(i+1)*5}초`]) },
